...
let radioButtons = document.querySelector(
  'input[name="radioButtons"]:checked'
).value;
...
d3.json("/static/chord/all_hierarchy_orig.json").then((data) => {
  const root = tree(
    bilink(
      d3
        .hierarchy(data)
  ...
  const node = container
    ...
    .text((d) => {
      if (radioButtons === "Chinese") {
        return d.data.name;
      } else {
        return d.data.english_translation;
      }
    })
    .each(function (d) {
      d.text = this;
...
Related terms: ${d.data.imports.map((i) => `\n${i}`)}`;
  ...
  function outed(event, d) {
    ...
    d3.selectAll(d.outgoing.map(([, d]) => d.text))
      .attr("fill", (l) => textColor(l.data.color + 1))
...
input.onkeyup = () => {
  ...
  if (term !== "") {
    ...
    var selected = d3.selectAll(".node").filter(function (d, i) {
      return (
        d.data.english_translation.toLowerCase().search(term.toLowerCase()) !=
          -1 || d.data.name.toLowerCase().search(term.toLowerCase()) != -1
      );
    });
    d3.selectAll(".node")
      .style("display", "none")
...
buttonsForm.onchange = () => {
  radioButtons = document.querySelector(
    'input[name="radioButtons"]:checked'
  ).value;
  d3.selectAll(".node")
    .text((d) => {
